...
- **Pipeline reuse:** `analyze()` (`src/analyze.c`) re-runs lexer→parser→pass1→pass2 per edit (full-document sync) and keeps the typed AST + symbols alive for queries. Features: live diagnostics (severity always Error), hover (`type_name` of the node), go-to-definition (top-level/module symbols via `resolved_sym->decl`, module members `mod.member` via `EXPR_FIELD.resolved_member` set in pass2, struct-literal type names, and union variant constructors `u.variant` → the union decl), completion (keywords + symbols + `.`/`::` members), and a CodeLens showing each `let`'s inferred type on the line above.
- **stdlib feed:** the installed stdlib (`FCC_STDLIB_DIR`, else the baked-in `$(datadir)/fcc/stdlib`, else `./stdlib`) is merged into every analysis; diagnostics are filtered to the open file. When the open document (or a sibling) *is* a feed file — most commonly because Go To Definition opened a stdlib module — the matching feed entry is dropped so the file isn't analyzed twice (which would trip a pass1 redefinition that silently halts pass2: no diagnostics *and* no hover/CodeLens). The dedup keys on the **canonical absolute path** (`realpath`, via `canon_path`), so the same on-disk file dedups regardless of how its path was spelled, while two *different* files that merely share a basename — a project's own `data.fc` vs the stdlib's — are correctly kept. (`src/lsp.c` defines `_DEFAULT_SOURCE` to expose `realpath` under `-std=c11`.)
...
- **Known v1 limits / follow-ups:** go-to-def does not reach block-locals (no `resolved_sym`), plain struct fields (`s.field` — fields carry no per-field source location), or members reached through a multi-level field chain (`a.b.member` — the field-name position is only computed when the object is a single identifier); field-name hover is approximate; completion over-offers (no flow-sensitive scoping); a small bounded per-analysis leak remains (~13KB, pass1's referenced Type/`type_params` mallocs — arena-convert them to close it); install targets are Linux-only.
