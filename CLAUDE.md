...
- **Known v1 limits / follow-ups:** completion over-offers (no flow-sensitive scoping); go-to-def on a union variant constructor lands on the union decl, not the individual variant (deliberate — see the wire test); install targets are Linux-only. *Pre-existing, unrelated:* opening `stdlib/data.fc` yields an empty CodeLens under `-O0` but works at `-O2` (the canonical `make test-lsp` build) — an optimization-level divergence present before this work, not yet diagnosed.
