...
- **Open items / known limits:** tracked in `spec/TODO.md` → "Editor / LSP server" (completion over-offering, variant-constructor go-to-def granularity, the per-keystroke stdlib re-parse, Linux-only install, and a pre-existing `-O0`-only empty-CodeLens divergence on `stdlib/data.fc`). User-facing limitations are summarized in `editors/vscode/README.md`.
